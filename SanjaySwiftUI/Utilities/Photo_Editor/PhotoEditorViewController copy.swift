...
@objcMembers public class PhotoEditorViewController: UIViewController {
    ...
    @IBAction func saveButtonTapped(_ sender: AnyObject) {
        ...
        if let data = image.png {
            if let pngImage = UIImage(data: data) {
                UIImageWriteToSavedPhotosAlbum( pngImage ,self, #selector(PhotoEditorViewController.image(_:withPotentialError:contextInfo:)), nil)
            }
        }
    }
    ...
    @IBAction func continueButtonPressed(_ sender: Any) {
        ...
        if let data = image.png {
            if let pngImage = UIImage(data: data) {
                photoEditorDelegate?.doneEditing(image: pngImage)
                self.dismiss(animated: true, completion: nil)
            }
        }
    }
