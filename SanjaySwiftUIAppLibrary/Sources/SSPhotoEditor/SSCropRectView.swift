...
@objc protocol SSCropRectViewDelegate: class {
...
@objcMembers class SSCropRectView: UIView, SSResizeControlDelegate {
    weak var delegate: SSCropRectViewDelegate?
    ...
    fileprivate let topLeftCornerView = SSResizeControl()
    fileprivate let topRightCornerView = SSResizeControl()
    fileprivate let bottomLeftCornerView = SSResizeControl()
    fileprivate let bottomRightCornerView = SSResizeControl()
    fileprivate let topEdgeView = SSResizeControl()
    fileprivate let leftEdgeView = SSResizeControl()
    fileprivate let rightEdgeView = SSResizeControl()
    fileprivate let bottomEdgeView = SSResizeControl()
    ...
    fileprivate func initialize() {
        ...
        // SSNote: to use resources from package, need to use "Bundle.module" instead of "bundle"
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        for subview in subviews where subview is SSResizeControl {
    ...
    fileprivate func cropRectWithResizeControlView(_ resizeControl: SSResizeControl) -> CGRect {
        var rect = frame
