...
@objcMembers open class SSCropView: UIView, UIScrollViewDelegate, UIGestureRecognizerDelegate, SSCropRectViewDelegate {
    ...
    open var keepAspectRatio = false {
        didSet {
            ssCropRectView.keepAspectRatio = keepAspectRatio
        }
    }
    ...
    open var resizeEnabled = true {
        didSet {
            ssCropRectView.enableResizing(resizeEnabled)
        }
    }
    ...
    fileprivate let ssCropRectView = SSCropRectView()
    ...
    fileprivate func initialize() {
        ...
        rotationGestureRecognizer = UIRotationGestureRecognizer(target: self, action: #selector(SSCropView.handleRotation(_:)))
        rotationGestureRecognizer?.delegate = self
        scrollView.addGestureRecognizer(rotationGestureRecognizer)
        
        ssCropRectView.delegate = self
        addSubview(ssCropRectView)
    ...
    open override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if let hitView = ssCropRectView.hitTest(convert(point, to: ssCropRectView), with: event) {
            return hitView
        }
    ...
    open func zoomedCropRect() -> CGRect {
        ...
        if (UIDevice.current.userInterfaceIdiom == UIUserInterfaceIdiom.pad || (orientation?.isPortrait ?? true) ) {
    ...
    @objc func handleRotation(_ gestureRecognizer: UIRotationGestureRecognizer) {
        ...
        case .began, .changed:
            ssCropRectView.showsGridMinor = true
        default:
            ssCropRectView.showsGridMinor = false
        }
    }
    ...
    fileprivate func cappedCropRectInImageRectWithCropRectView(_ cropRectView: SSCropRectView) -> CGRect {
        ...
        if cropRect.minX < editingRect.minX - 5.0 ||
            ...
            cropRect.maxY > editingRect.maxY + 5.0 {
                UIView.animate(withDuration: 1.0, delay: 0.0, options: .beginFromCurrentState, animations: { [unowned self] in
                    self.zoomToCropRect(self.ssCropRectView.frame)
                    }, completion: nil)
        }
    }
    ...
    // MARK: - SSCropView delegate methods
    func cropRectViewDidBeginEditing(_ view: SSCropRectView) {
        resizing = true
    }
    
    func cropRectViewDidChange(_ view: SSCropRectView) {
    ...
    func cropRectViewDidEndEditing(_ view: SSCropRectView) {
        resizing = false
        zoomToCropRect(ssCropRectView.frame)
    }
